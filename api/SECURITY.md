...
③ uid かメールが許可リストにあるか  ★ここだけが門番★  → 無ければ 403
④ 回数の上限                        ③を通った人への制限
```
...
| メールでの許可 | トークンの `email_verified` が真のものだけ。Google ログインなら常に真。匿名には無い |
...
**「CORS が代わりに守っている」わけではない。** キーが使われる相手は
`identitytoolkit.googleapis.com` で、うちの API の CORS はそこには届かない。
Firebase は任意のドメインで動く前提なので、あのエンドポイントは**どの
オリジンからのブラウザ呼び出しも受ける**。つまりリファラー制限を掛けない
以上、他人のページから匿名アカウントを作られること自体は止まっていない。

それでも掛けないのは、**被害を抑えているのが量の上限だから**。
サインアップは100件/1時間で頭打ちになり、溜まったアカウントは30日で
自動削除される。実害は「クォータを食い潰されて、正規の新規サインアップが
一時的に通らなくなる」程度に収まる。

その程度の効果のために本番オリジンを1つに固定する制約を負うより、
量の上限で抑えるほうが運用が楽だと判断した。
