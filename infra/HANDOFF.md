...
このリポジトリに関わる未解決事項は次の2つ。どちらも**実機でしか確かめられない**。

- **ワーカーのメモリ使用率が未測定。** PR #12 で `worker_entrypoint.py` を
  `subprocess.run` から `Popen` に変えた。理屈の上では従来より軽い（旧実装は
  標準エラーの全文をメモリに持っていた）が、過去に OOM を踏んでいる箇所なので
  実測したい
- **完了通知（`/internal/dispatch`）が2件目で効くか未確認。** 1件だけ試しても
  症状が出ない（通知に失敗しても `GET /jobs/{id}` 側の経路が枠を回収するため）。
  2件目がいつまでも `queued` なら、そこが失敗している
