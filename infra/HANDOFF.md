...
1 件の内訳（実測、2026-09-14、椅子の写真）。以前は 522 秒だったのが **226 秒**になった。

| 工程 | 秒 | 前 |
|---|---|---|
| コンテナ起動 | 13 | 25 |
| 重みのダウンロード（10.9GB、VPC 経由） | 14 | （gcsfuse で読みながら 263） |
| import・準備（torch などの .so をイメージから読む） | 31 | 15 |
| テクスチャモデルの読み込み（/tmp から） | 18 | 185 |
| 形状モデルの読み込み | 4 | 78 |
| 形状生成・面削減 | 26 | 30 |
| テクスチャ生成 | 103 | 169 |
| 保存・終了 | 17 | 20 |
...
| 打ち手 | 効果 | 前提 |
|---|---|---|
| 常駐サーバー化 | 2 件目以降の読み込み（約 130 秒）が消える | **リクエストが時間的に固まること** |
| テクスチャ生成のステップ数を減らす | 106 秒 → 60 秒前後 | 品質との天秤 |
