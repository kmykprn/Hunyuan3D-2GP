...
**このリポジトリで直すものは無い。** 残っているのは GCP プロジェクト共通の1つ。

- **OAuth 同意画面が「テスト中」のまま。** Google ログインは使えるが、
  テストユーザーに登録したアカウントしか通らない（上限100人）。人に配る前に
  本番へ切り替える必要がある。切り替えにはホームページとプライバシーポリシーの
  URL が要り、**そのプライバシーポリシーを書いて置くのはクライアント側**
  （`roomplanner-web` の `public/privacy.html`）。**公開ステータスもテストユーザーの
...
1 件の内訳（実測、2026-09-14、椅子の写真）。以前は 522 秒だったのが **166 秒**になった。

| 工程 | 秒 | 前 |
|---|---|---|
| コンテナ起動 | 8 | 25 |
| 重みのダウンロード（10.9GB、VPC 経由） | 14 | （gcsfuse で読みながら 263） |
| import・準備（torch などの .so をイメージから読む） | 30 | 15 |
| テクスチャ・形状モデルの読み込み（/tmp から） | 22 | 263 |
| 形状生成・面削減 | 25 | 30 |
| テクスチャ生成（delight 6 / UV 展開 11 / 拡散 21 / 焼き込み 1 / 穴埋め 4） | 45 | 169 |
| 保存・終了 | 15 | 20 |

テクスチャ生成 104 秒のうち 59 秒は「頂点ベースの穴埋め」を Python のループで回していた時間で、
numpy / scipy の疎行列に書き直して 4 秒になった（`mesh_processor.py`、埋める画素の集合は元と一致）。
拡散のステップ数・視点数・解像度は環境変数（`TEXTURE_STEPS` / `TEXTURE_VIEWS` / `TEXTURE_SIZE`）で
変えられるが、実測ではほとんど効かなかったので既定のまま。

テクスチャモデルの読み込みに見えていた 75 秒のうち 57 秒は、rembg が依存する pymatting の
numba JIT コンパイルだった（`NUMBA_DISABLE_JIT=1` で消える。この生成ではコンパイルされる関数を使わない）。
残る「import・準備 31 秒」は torch などの共有ライブラリをイメージから初めて読む時間（image streaming、約 60MB/s）。

重みは `hub-fp16/` にあり、ワーカーが起動時に Direct VPC 経由で /tmp へ落とす。
既定の経路だと 60MB/s で頭打ちだったのが、VPC + Private Google Access で 700MB/s 出る。

まだ残っている打ち手:

| 打ち手 | 効果 | 前提 |
|---|---|---|
| 常駐サーバー化 | 2 件目以降の起動・読み込み（約 75 秒）が消える | **リクエストが時間的に固まること** |
| テクスチャの解像度 2048 → 1024 | −10 秒前後 | 寄ったときの細部が甘くなる |
